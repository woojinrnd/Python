...
class Buttons:
    ...
    def display_buttons(self, frame):
        for b_index, button_value in self.buttons.items():
            ...
            cv2.putText(frame, button_text, (x + self.x_margin, bottom_y - self.y_margin), self.font, self.text_scale, text_color, self.text_thick)
    ...
    def button_click(self, mouse_x, mouse_y):
        for b_index, button_value in self.buttons.items():
            ...
            if inside > 0:
                ...
                self.button[b_index]["active"] = new_status


    def active_buttons_list(self):
        active_list = []
        for b_index, button_value in self.button.items():
            active = button_value["active"]
